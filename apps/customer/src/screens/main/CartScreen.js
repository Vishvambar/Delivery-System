...
    
    // Get delivery fee from vendor (assume first item's vendor for simplicity)
    if (cart.vendorId) {
      // You could fetch vendor details here to get actual delivery fee
      setDeliveryFee(2.99); // Default delivery fee
    }
    
...
    // Add default coordinates for now (you could implement proper geocoding later)
...
    const subtotal = cart.items.reduce((sum, item) => sum + (item.price * item.quantity), 0);
    const minimumOrder = 10; // You could get this from vendor data
    
...
      // Emit socket event for real-time vendor notification
...
        onPress={() => dispatch(removeFromCart(item.menuItemId))}
      >
